...
router.get('/', (_, res) => {
  ...
  db.query('SELECT u.id, u.isAdmin, u.nome, u.telefone, u.email, u.dataCadastro, c.salario, e.provincia, e.municipio, e.bairro, e.numeroDaCasa from usuarios u join colaboradores c on (c.usuarios_id = u.id) join enderecos e on (c.enderecos_id = e.id) order by u.id desc LIMIT 7 ', (error, results) => {
    if (error) {
      throw error
    }
    res.send({
